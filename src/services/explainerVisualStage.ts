...
//                reaction pattern's ping-pong (playbackOrderFor) oscillates A->B->A. Reaction
//                steps are oscillating MOVEMENTS (propeller, flutter), never expression arcs.
...
export function resolveMotionPlan(motion: SceneMotion | undefined, resolvedNano: boolean): MotionPlan {
  ...
  if (motionClass === 'content' || motionClass === 'semi') {
    ...
    // ping-pong oscillates base+step A->B->A) — never to a static or wiggle.
