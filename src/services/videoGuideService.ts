...
// Continuation guide behind POST /api/prompt/from-video (2026-07-25): Gemini WATCHES the user's
// reference clip and writes ONE continuation prompt grounded in the actual final-moment state —
// subject + position, motion direction/speed, camera, lighting, audio ambience. This is the
// video-aware complement to the sight-unseen dispatch interceptor (enhanceForDispatch): the
// guide shapes what the user TYPES, the interceptor still cleans up whatever finally submits.
//
...
const BASE_INSTRUCTION =
  'You are helping a user continue a short AI-generated video clip. Watch the clip, especially ' +
  'its FINAL moments. Write ONE prompt for an AI video generation model that continues this clip ' +
  'directly from its final frame: NO cut, NO transition — the action resumes seamlessly. Ground ' +
  'the prompt in what you actually see and hear at the end of the clip: the subject and its ' +
  'exact position, direction and speed of motion, camera placement and movement, lighting, and ' +
  'audio ambience — then the new action. If the clip has no clear subject, describe the scene ' +
  'and camera instead. One paragraph. Output only the prompt text, no preamble or explanation.';

const HINT_INSTRUCTION_SUFFIX =
  ' The user\'s intended direction for the continuation is given after the clip — realize THEIR ' +
  'idea as the new action while keeping every continuity element grounded in the footage.';
