...
describe('processOpenAIGeneration — faceswap', () => {
  ...
  it('CSAM flagged → markQuarantined + refundCredits, never markCompleted', async () => {
    (generateFaceswap as jest.Mock).mockResolvedValue('generations/gen-fs-1.png');
    (scanForCsam as jest.Mock).mockResolvedValue({ flagged: true });

    await processOpenAIGeneration(FACESWAP_JOB);

    expect(markQuarantined).toHaveBeenCalledWith('gen-fs-1');
    expect(refundCredits).toHaveBeenCalledWith('user-1', 5, 'csam-quarantine-openai-gen-fs-1');
    expect(markCompleted).not.toHaveBeenCalled();
    expect(deleteRawFaceUploads).not.toHaveBeenCalled();
  });
